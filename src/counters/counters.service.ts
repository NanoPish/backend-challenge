...
export class CountersService {
    ...
    async increment(data: IncrementCounterInput): Promise<Counter> {
        const oldCounterValue = CountersService.counters.get(data.key);
        const incrementCount = Math.round(data.value);
        const newCounterValue = oldCounterValue !== undefined ? oldCounterValue + incrementCount : incrementCount;
    
        CountersService.isSorted = false;
        CountersService.counters.set(data.key, newCounterValue);
        
        const resultingCounter = new Counter(data.key, newCounterValue);
    ...
    async findOneByKey(key?: string): Promise<Counter> {
        ...
        const resultValue = CountersService.counters.get(key);
